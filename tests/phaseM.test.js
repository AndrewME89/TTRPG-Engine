...
test('Run Session rebuildTimers uses saveStateQuiet for Delete', () => {
  ...
  const delBlock = rebuildFn.slice(delIdx, delIdx + 120);
...
test('War Machine Reset uses saveStateQuiet not full saveState', () => {
  ...
  const resetBlock = warMach.slice(resetIdx, resetIdx + 180);
...
test('War Machine timer Delete saves state', () => {
...
test('Set HP button uses saveStateQuiet', () => {
  ...
  const block = pcChar.slice(idx, idx + 220);
...
console.log('\n  Section 7: Compendium local state preserved');
test('renderReference uses local rs state not plugin.state.search', () => {
  ok(refFn.includes('const rs = {') && refFn.includes('search') && refFn.includes('limit'),
    'local rs state missing from renderReference');
  notOk(refFn.includes('plugin.state.search ='), 'renderReference still writing to plugin.state.search');
});
test('renderReference rebuild does not call plugin.saveState', () => {
  const idx = refFn.indexOf('const rebuild = async () =>');
  ok(idx !== -1, 'rebuild function missing from renderReference');
  const fnBody = refFn.slice(idx, idx + 5000);
  notOk(fnBody.includes('plugin.saveState'), 'renderReference rebuild calling plugin.saveState');
});
test('renderLibrary stays a lightweight alias', () => {
  ok(libFn.includes('return renderReference'), 'renderLibrary should delegate to renderReference');
});
